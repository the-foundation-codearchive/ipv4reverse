...
for octet_three in 0 $(seq 1 254);do 
            ...
            #[[ $(sudo netstat -puteen 2>/dev/null |wc -l ) -ge 768  ]]     && (sleep 12  ;  echo "GEN_conn_throttled 12 s FOR ${octet_one}.${octet_two}.${octet_three} CONNs: "$(sudo netstat -puteen 2>/dev/null |wc -l ) >&2 )
...
#     )
     (  
         ...
         (
            ...
            echo 0|bash /tmp/bash-logger/log-to-influxdb2.sh "${LOGTOINFLUXURL}" buildstatus FALSE buildtime "${LOGTOINFLUXTOKEN}" OCT_${octet_one}_${octet_two}_${octet_three}_$STATSTARGET &
            ...
            myend=$(date -u +%s)
            export INFLUX_MEASUREMENT=buildtime            
            echo $(($myend-$mystart))|bash /tmp/bash-logger/log-to-influxdb2.sh "${LOGTOINFLUXURL}" buildstatus FALSE buildtime "${LOGTOINFLUXTOKEN}" OCT_${octet_one}_${octet_two}_${octet_three}_$STATSTARGET &
