...
for octet_three in 0 $(seq 1 254);do 
            ...
            [[ $(sudo netstat -puteen 2>/dev/null  |wc -l ) -ge 3072 ]]    && (sleep 66  ;  echo "GEN_throttled 66s s FOR ${octet_one}.${octet_two}.${octet_three} CONNs: "$(sudo netstat -puteen 2>/dev/null  |wc -l ) >&2 )
            [[ $(sudo netstat -puteen 2>/dev/null  |wc -l ) -ge 2048 ]]    && (sleep 42  ;  echo "GEN_throttled 42s s FOR ${octet_one}.${octet_two}.${octet_three} CONNs: "$(sudo netstat -puteen 2>/dev/null  |wc -l ) >&2 )
            [[ $(sudo netstat -puteen 2>/dev/null  |wc -l ) -ge 1536 ]]    && (sleep 13  ;  echo "GEN_throttled 13s s FOR ${octet_one}.${octet_two}.${octet_three} CONNs: "$(sudo netstat -puteen 2>/dev/null  |wc -l ) >&2 )
            [[ $(sudo netstat -puteen 2>/dev/null  |wc -l ) -ge 1024 ]]    && (sleep 5   ;  echo "GEN_throttled  5s s FOR ${octet_one}.${octet_two}.${octet_three} CONNs: "$(sudo netstat -puteen 2>/dev/null  |wc -l ) >&2 )
