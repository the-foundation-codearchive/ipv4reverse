...
for octet_three in 0 $(seq 1 254);do 
            ...
            while ( [[ $(netstat -puteen 2>/dev/null |grep -e ^tcp -e ^udp |wc -l ) -ge 1024 ]] ) ;do echo "GEN_sleeping until less than 1024 connections , CONNS: "$(sudo netstat -puteen 2>/dev/null |wc -l ) >&2;sleep 10;done
            ...
            [[ $(sudo netstat -puteen 2>/dev/null |wc -l ) -ge 666  ]]     && (sleep  9  ;  echo "GEN_conn_throttled  9 s FOR ${octet_one}.${octet_two}.${octet_three} CONNs: "$(sudo netstat -puteen 2>/dev/null |wc -l ) >&2 )
